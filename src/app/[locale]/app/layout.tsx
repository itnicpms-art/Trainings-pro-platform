...
export default async function AppLayout({ children, params }: { children: ReactNode; params: LocaleParams }) {
  ...
  return <div className="min-h-screen bg-slate-50"><aside className="fixed inset-y-0 left-0 z-40 hidden w-72 lg:block"><AppSidebar locale={locale} translations={dictionary.shell} dashboardTranslations={dictionary.app.dashboardShell} dashboardVariant={dashboardVariant} activeProfileName={activeProfile.display_name} activeProfileStatus={activeStatus} /></aside><div className="lg:pl-72"><Topbar area="app" title="Trainings PRO" locale={locale} translations={dictionary.shell} dashboardTranslations={dictionary.app.dashboardShell} dashboardVariant={dashboardVariant} languageLabel={dictionary.language.label} accountName={activeProfile.display_name || user.email || "Trainings PRO"} profileLabel={profileLabel} activeProfileName={activeProfile.display_name} activeProfileStatus={activeStatus} /><main className="p-4 sm:p-5 lg:p-6">{children}</main></div></div>;
}
