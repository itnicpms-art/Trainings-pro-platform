...
export default async function AppLayout({ children, params }: { children: ReactNode; params: LocaleParams }) {
  ...
  return <div className="min-h-screen bg-slate-50"><aside className="fixed inset-y-0 left-0 z-40 hidden w-64 lg:block"><AppSidebar locale={locale} translations={dictionary.shell} dashboardTranslations={dictionary.app.dashboardShell} dashboardVariant={dashboardVariant} showOrganizationContext={dashboardContext.showOrganizationContext} activeProfileName={activeProfile.display_name} activeProfileStatus={activeStatus} /></aside><div className="lg:pl-64"><Topbar area="app" title="Trainings PRO" locale={locale} translations={dictionary.shell} dashboardTranslations={dictionary.app.dashboardShell} dashboardVariant={dashboardVariant} showOrganizationContext={dashboardContext.showOrganizationContext} languageLabel={dictionary.language.label} accountName={activeProfile.display_name || user.email || "Trainings PRO"} profileLabel={profileLabel} activeProfileId={activeProfile.id} activeProfileName={activeProfile.display_name} activeProfileStatus={activeStatus} profiles={profileOptions} profileSwitcherTranslations={profileSwitcherTranslations} /><main className="p-3 sm:p-4 lg:p-5">{children}</main></div></div>;
}
