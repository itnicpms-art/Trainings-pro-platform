...
export default async function AdminContentPage({ params }: { params: LocaleParams }) {
  const locale = await resolveLocale(params);
  const dictionary = await getDictionary(locale);
  const t = dictionary.admin.content;
  const pending = dictionary.admin.common.comingSoon;
  return <div className="mx-auto max-w-6xl"><PageHeading eyebrow={t.eyebrow} title={t.title} description={t.description} /><AdminSection title={t.title} description={t.description}><div className="mb-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{t.sections.map((section) => <div key={section} className="rounded-xl border border-slate-200 p-3"><p className="text-sm font-semibold text-[#06113B]">{section}</p><Badge variant="secondary" className="mt-3">{pending}</Badge></div>)}</div><AdminEmptyState icon={LibraryBig} title={t.emptyTitle} description={t.emptyDescription} label={pending} /></AdminSection></div>;
}
