...
- **`revoke_academic_program_staff_role`** locked the `profile_roles` row (identifiable by a caller-supplied `assignment_id` alone) before authorizing — fixed the same way; the post-lock revalidation is a "still exists" check, since `scope_type`/`scope_id`/`role_id` are immutable on this table (nothing ever `UPDATE`s a `profile_roles` row, only `INSERT`/`DELETE`).
- **`create_academic_group`**, **`move_student_group_membership`**, **`end_student_group_membership`** were audited and found already correct without changes: the first authorizes on a raw input parameter before any lock exists; the latter two authorize on `academic_profile_contexts.academic_program_id`, which was traced across every write in this file and confirmed never reassigned to a different value on an existing row (`add_student_to_group`'s only write to it is itself guarded to always reaffirm the same program id) — `end_student_group_membership` additionally has no separate lock step at all, relying correctly on its `UPDATE ... WHERE status = 'active'` clause as an atomic lock-and-guard in one statement. Defensive post-lock re-checks of `academic_program_id` were still added to `move_student_group_membership`/`set_primary_group_membership` for consistency with the other functions, even though not exploitable today.
