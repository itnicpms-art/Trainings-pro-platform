...
**Root cause, confirmed on re-inspection:** `end_student_group_membership`'s `UPDATE` and its audit-trail `INSERT` into `student_group_membership_audit_events` ran as two statements inside one `plpgsql` function body with no isolation between them. Every column, constraint, and trigger on both tables was re-checked against this exact UPDATE and INSERT and none is provably violated by the current schema — but that same lack of isolation means that *if* the audit `INSERT` ever throws for any reason (a future schema drift, an environment-specific constraint, or anything else not reproducible by static review), `plpgsql` rolls back the **entire function call as one transaction**, silently undoing the `UPDATE` that had already renamed the row to `inactive`. This exactly matches the reported symptom: the membership state change is attempted and then invisibly erased by an unrelated logging failure, with no application-level error surfacing because the RPC call itself throws a raw Postgres exception that a client retry would simply repeat.

**Fix:** the `UPDATE` and the audit `INSERT` are now each wrapped in their own `begin...exception when others...end` block. The `UPDATE`'s handler re-raises (folding the real `sqlstate`/`sqlerrm` into the message) since there is nothing valid to return if the state change itself cannot happen. The audit `INSERT`'s handler now downgrades to `raise warning` instead of `raise exception`: a warning is logged server-side (visible in Supabase's own Postgres logs) but never aborts the transaction and never reaches the HTTP/client layer, so the membership still ends successfully even if its audit record cannot be written. The end-user-facing error mapping in `mutate-student-group-membership.ts` is unchanged — raw DB details were never, and are still never, exposed to end users.
