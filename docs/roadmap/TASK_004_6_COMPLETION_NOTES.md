...
**Root cause: not yet confirmed with concrete evidence.** `end_student_group_membership`'s `UPDATE` and its audit-trail `INSERT` into `student_group_membership_audit_events` were re-checked column-by-column and constraint-by-constraint (including both conditional triggers on `academic_profile_contexts`, which only fire on columns this UPDATE never touches), and cross-compared against the structurally identical audit inserts in `add_student_to_group`, `move_student_group_membership`, and `set_primary_group_membership` in the same migration. No provable constraint, trigger, RLS, or privilege violation was found on the schema as committed. The audit `INSERT` failing and rolling back the preceding `UPDATE` remains the leading hypothesis — a plpgsql function body is one transaction, so an uncaught exception from either statement aborts the whole call and rolls back everything before it, which would exactly match the reported symptom — but it is a hypothesis, not a confirmed cause, since no live Postgres error has been captured yet.

**An earlier revision of this fix (reverted):** briefly downgraded the audit `INSERT`'s failure handling to `raise warning` so the membership would end successfully even if its audit record could not be written. This was rejected: administrative membership mutations must keep the state change and its audit record atomic — both succeed or both roll back — so a missing audit trail is never an acceptable trade-off for a "successful" mutation, even temporarily.

**Current change:** the `UPDATE` and the audit `INSERT` are each wrapped in their own `begin...exception when others...end` block, but both handlers still `raise exception` — atomicity is unchanged from the original behavior; an uncaught error anywhere still aborts and rolls back the whole call. What the handlers add is diagnostics: each captures `GET STACKED DIAGNOSTICS` (`RETURNED_SQLSTATE`, `CONSTRAINT_NAME`, `TABLE_NAME`, `MESSAGE_TEXT`), writes the full detail to the server-side Postgres log only via `raise log` (never sent to the client), and re-raises a stable, greppable stage code — `END_MEMBERSHIP_UPDATE_FAILED` or `END_MEMBERSHIP_AUDIT_FAILED` — identifying which statement actually failed. `mutate-student-group-membership.ts` also temporarily logs the raw RPC error server-side (`console.error`, never sent to the browser) for the `end` intent, so the stage code shows up in application logs too. End-user-facing behavior is unchanged: `mapMembershipError` still maps any unrecognized `22023` message to the existing safe, localized "invalid" reason. Both diagnostic additions are marked temporary and should be removed once a live reproduction confirms the exact failing statement and its real cause.
