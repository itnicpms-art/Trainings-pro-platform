...
- Added migration `013_student_group_membership_end_date_fix.sql` (forward-only; see investigation below) without modifying migration 012 or any earlier migration.
- Added migration `014_move_membership_audit_before_snapshot_fix.sql` (forward-only; see final QA below) without modifying migrations 012 or 013.
...
Runtime QA first reported that after clicking End, the student kept appearing as an active member with no error shown. An initial pass (documented in the paragraph below, kept for the record) traced every application layer without database access and found no bug in the RPC, schema validation, or revalidation call. QA then confirmed directly against Supabase that the `academic_profile_contexts` row's `status` genuinely never left `'active'` — proving this was a real backend failure, not a stale UI read.

*Superseded first pass:* `end_student_group_membership`'s SQL was re-read line by line (correctly targets `membership_id`, updates only an `active` row, raises on mismatch); the "End" form's hidden fields were re-verified wired correctly; Zod v4's `discriminatedUnion` stripping/parsing behavior was verified directly with a standalone script; and this fork's actual `revalidatePath`/caching docs (`node_modules/next/dist/docs/`) were read directly, confirming `revalidatePath` from a Server Function updates a currently-viewed path immediately under this project's caching model. Two independent, low-risk improvements were made at that point — defensive payload verification in `mutateStudentGroupMembership` (treats a returned row whose `status`/`is_primary` doesn't match what the intent claims as an error, instead of trusting "no Postgres error" alone) and splitting the panel into an active list plus a collapsed "Membership history" disclosure for ended rows — but the root cause itself was not yet confirmed.

**An earlier revision of this fix (reverted):** briefly downgraded the audit `INSERT`'s failure handling to `raise warning` so the membership would end successfully even if its audit record could not be written. This was rejected: administrative membership mutations must keep the state change and its audit record atomic — both succeed or both roll back — so a missing audit trail is never an acceptable trade-off for a "successful" mutation, even temporarily. A second revision added `GET STACKED DIAGNOSTICS`/`raise log` instrumentation and stable `END_MEMBERSHIP_UPDATE_FAILED`/`END_MEMBERSHIP_AUDIT_FAILED` stage codes to both statements' exception handlers, while keeping both fail-closed (`raise exception`, atomicity unchanged) — this was meant to capture the real error on the next reproduction, since static review alone could not find a provable constraint/trigger violation.

**Root cause: confirmed from live Supabase data.** Direct inspection of the deployed database showed `public.end_student_group_membership` was still running a version that computed `ended_at = current_date` unconditionally — none of this task's date-handling fixes had actually reached the live function. For the affected membership (`id = 078b4a53-7c7d-4066-87a0-cb7b58649c61`, `started_at = 2026-10-01`), calling it while `current_date = 2026-09-14` set `ended_at = 2026-09-14`, which is earlier than `started_at` and violates `academic_profile_contexts_date_order_check` (migration 004, unmodified: `ended_at is null or started_at is null or ended_at >= started_at`), expected `SQLSTATE 23514`. The `UPDATE` fails on this check constraint before the audit `INSERT` ever runs — the audit `INSERT` is confirmed **not** the cause of this failure, and is no longer treated as a hypothesis.

**Forward-only fix:** per `AGENTS.md` section 3 ("Never modify an existing committed migration"), migration 012 has been restored in the working tree to exactly its originally-committed content (from commit `92474c9`) — the diagnostic instrumentation and every in-place date-handling edit previously stacked into that file have been removed from it. The actual fix now lives in a new migration, `013_student_group_membership_end_date_fix.sql`, which `create or replace function`s `end_student_group_membership` and `move_student_group_membership` with `ended_at = greatest(current_date, existing_membership.started_at)` — `move_student_group_membership` has the identical old-row-ending step and the identical exposure, so it is fixed in the same migration to avoid silently reintroducing that already-fixed bug when migration 012 reverts to its original content. Neither function wraps its `UPDATE`/`INSERT` pair in any exception handler: an uncaught error in either statement aborts the whole call and rolls back everything before it, which is plpgsql's plain default behavior — no diagnostics, stage codes, or logging were carried into the new migration, since the root cause is now confirmed rather than hypothesized. The temporary `console.error` diagnostic in `mutate-student-group-membership.ts` has likewise been removed; end-user-facing behavior is unchanged.

## Final runtime QA (post migration 013)

After deploying migration 013, runtime QA passed for: end membership including a future-dated `started_at`; add membership; move membership; primary-membership preservation; history preservation; and audit event creation on every mutation.

QA did surface one remaining audit-correctness issue, scoped to `move_student_group_membership` only:

**Issue:** the move audit event's `before_snapshot` was recorded as `to_jsonb(ended_row)`, but `ended_row` is populated by `returning context.* into ended_row` on the `UPDATE` that had *already* set `status = 'inactive'` and `is_primary = false` on the source row — so `before_snapshot` captured the row's state *after* that update, not before it. QA reproduced this directly: moving a membership that was `active`/`is_primary = true` recorded `before_status = 'inactive'`, `before_is_primary = false` in the audit row, when the true pre-move state was `active`/`true`.

**Fix (`014_move_membership_audit_before_snapshot_fix.sql`):** `before_snapshot` now uses `to_jsonb(existing_membership)` instead — `existing_membership` is populated by the `select ... for update` earlier in the function, before the ending `UPDATE` runs, so it correctly holds the row's true pre-mutation state. `after_snapshot` (`to_jsonb(new_row)`) and `old_academic_group_id`/`new_academic_group_id` are unchanged. No other statement in `move_student_group_membership` changed: `ended_at = greatest(current_date, existing_membership.started_at)` and the lack of any exception handler around the `UPDATE`/`INSERT` pair (an uncaught error in either statement still aborts and rolls back the whole call) both carry forward from migration 013 unmodified.
