...
- Nothing needs to be deleted to preserve history: a **move** ends the old row (`status = 'inactive'`, `ended_at = greatest(current_date, started_at)`) and inserts a new one; an **end** only changes `status`/`ended_at`/`is_primary` on the existing row. `greatest(...)` guards against a future-dated `started_at`: ending or moving a not-yet-started membership must never produce `ended_at < started_at`, which `academic_profile_contexts_date_order_check` (migration 004) rejects. A profile naturally accumulates multiple rows over time — that multiplicity already is the history.
